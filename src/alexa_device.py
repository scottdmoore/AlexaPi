...
class AlarmManager:
    ...
    def set_alert(self, token, alert_type, scheduled_time):
        ...
        try:
            ...
            timer_thread.start()
            sunrisetime = time_difference -(30*60)
            sunrise_duration = 30*60
            if sunrisetime < 0:
                sunrise_duration = time_difference 
                sunrisetime = 1
                print "!!!!!!! SUN IS RISING NOW"
            if not self.alexa_device:
              print "Alexa device not set. Could not set alarm"
              return False
            timer_sunrise_thread = threading.Timer(sunrisetime, self.alexa_device.start_sunrise, args=(sunrise_duration,))
            timer_sunrise_thread.start()
            stop_event = threading.Event()
            self.alerts[token] = {
                'type': alert_type,
                'scheduled_time': scheduled_time,
                'timer_thread': timer_thread,
                'timer_sunrise_thread': timer_sunrise_thread,
    ...
    def start_alert(self, token):
        ...
        if True:
            ...
            self.alexa_device.alexa.get_and_process_response(stream_id)
            raw_audio = self.audio.get_weather_request()
            if raw_audio is None:
                print "No Audio"
            else:
                # TODO make it so the response can be interrupted by user if desired (maybe start a thread)
                stream_id = self.alexa_device.alexa.start_recognize_event(raw_audio)
                self.alexa_device.alexa.get_and_process_response(stream_id)
        else:
...
class SpeechManager:
    """ This object manages all alarms and timers sent via the SpeechSynth interface.
    """
    def __init__(self, audio):
        """ Initializes the SpeechManager object. Requires an AlexaAudio object to speak. The
            AlexaCommunication object must be specified in a separate function call.
...
class AlexaDevice:
    ...
    def __init__(self, alexa_config):
        ...
        self.speech_manager = SpeechManager(self.alexa_audio_instance)
        ...
        self.alexa_recognition_thread = None
        self.sunrise_thread = None
        self.sunrise_stop = None
    ...
    def start_sunrise(self, time):
        print "Sunrise thread triggered"
        if not self.sunrise_thread:
          self.sunrise_stop = threading.Event()
          self.sunrise_thread = threading.Thread(target=strandtest.doSunrise, args=(time,self.sunrise_stop))
          self.sunrise_thread.start()

    def stop_sunrise(self):
        if self.sunrise_thread:
          self.sunrise_stop.set()
          self.sunrise_thread = None


    def user_initiate_audio(self):
        if self.alexa_recognition_thread:
            print "Outstanding request"
            self.alexa_audio_instance.play_alexa_outstanding()
            return
        self.alexa_audio_instance.play_alexa_yes()
        ...
        # TODO make it so the response can be interrupted by user if desired (maybe start a thread)
        self.alexa_recognition_thread = threading.Thread(target=self.send_rec, args=(raw_audio,))
        self.alexa_recognition_thread.start()

    def send_rec(self, raw_audio):
    ...
    def get_context(self):
        ...
        context_speech = self.speech_manager.get_speech_context()
        
        return [context_audio, context_alerts, context_speaker, context_speech]
    ...
    def process_response(self, message):
        ...
        print("Messages received: " + str(message['content']))
        ...
        if len(message['attachment']) > 1:
            print "Too many attachments (%d)" % len(message['attachment'])
        ...
        for content in message['content']:
            ...
            print "Received a directive: "+ str(header)
    ...
    def process_directive_speech_synthesizer(self, content, attachment):
        ...
        if name == 'Speak':
            ...
            token = payload['token']
            self.speech_manager.token = token
            self.speech_manager.speaking = True
            ...
            self.speech_manager.speaking = False
