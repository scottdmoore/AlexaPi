...
def read_from_downstream(boundary, data):
    ...
    #index = data.rfind(boundary)
    #data = data[index:]
    #new_data = data[:index].split(boundary)
    return data, []
...
class AlexaConnection:
    ...
    def init_connection(self):
        ...
        stream_id = self.send_event(header, method='POST')
    ...
    def start_downstream(self, lock=False):
        ...
        #print "Received downstream response: "+ str(downstream_response.read())
    ...
    def downstream_thread(self):
        ...
        while not self.thread_stop_event.is_set():
            if len(actual_stream.data) > 1:
                ...
                if len(new_data) > 0:
                    for mess in new_data:
                        message = parse_data(mess, self.downstream_boundary)
    ...
    def ping_thread(self):
        ...
        while not self.thread_stop_event.is_set():
            ...
            while not self.thread_stop_event.is_set() \
                    and (time.mktime(time.gmtime()) - start_sleep_time) < 30:
                time.sleep(1)
        print("Closing ping thread.")

    def reset_connection(self):
        self.lock.acquire()
        print "Acquired http lock after a failed ping"
        self.connection.close()
        self.lock.release()
        print "Released http lock after a failed ping"
    ...
    def close(self):
        ...
        print "Acquiring http2 connection lock"
        self.lock.acquire()
        print "Closing http2 connection"
        self.connection.close()
    ...
    def send_request(self, method, path, body=None, path_version=True):
        ...
        # Send actual request
        #self.lock.acquire()
        print "Acquired http lock for making a request"
        try:
          if body is not None:
          ...
          print "Received stream closed error"
          #self.lock.release()
          print "Released http lock after a failed request"
          return
        #self.lock.release()
        print "Released http lock after making a request"

        return stream_id

    def send_event(self, header, payload=None, audio=None, method='POST'):
        ...
        print "Sending event: " + str(header)
        ...
        stream_id = self.send_request(method, '/events', body=body_string)
        while not stream_id:
          print "Resetting connection"
          self.reset_connection()
          print "Trying to send event again " + str(header)
          stream_id = self.send_request(method, '/events', body=body_string)
        return stream_id
    ...
    def get_response(self, stream_id):
        ...
        """
        #self.lock.acquire()
        print "Acquired http lock for getting a response"
        result = self.connection.get_response(stream_id)
        #self.lock.release()
        print "Released http lock after getting a response"
        return result
